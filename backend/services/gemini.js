...
export async function chatCallAgent(sessionId, message, availableSchemes = [], userProfile = {}) {
  const memory = getMemory(sessionId);
  // A spoken answer such as "Telangana" is not part of the saved web profile.
  // Keep it for this call so the simulator (and the next prompt) can move forward.
  const savedCallProfile = callSessionProfiles[sessionId] || {};
  const mentionedState = getMentionedIndianState(message);
  userProfile = { ...savedCallProfile, ...userProfile, ...(mentionedState ? { state: mentionedState } : {}) };
  const requestedScheme = getRequestedCallScheme(message);
  const previousScheme = userProfile.callScheme;
  if (requestedScheme) userProfile.callScheme = requestedScheme;

  const selectedScheme = availableSchemes.find(s => s.id === requestedScheme) ||
    availableSchemes.find(s => s.id === userProfile.callScheme);

  // One reusable eligibility interview for every scheme in the database.
  if (selectedScheme) {
    const questions = getEligibilityQuestions(selectedScheme);
    const isNewScheme = requestedScheme && requestedScheme !== previousScheme;
    const currentIndex = isNewScheme ? 0 : (userProfile.callQuestionIndex ?? 0);

    userProfile.callScheme = selectedScheme.id;
    userProfile.callAnswers = isNewScheme ? {} : (userProfile.callAnswers || {});

    if (isNewScheme) {
      userProfile.callQuestionIndex = 0;
      callSessionProfiles[sessionId] = userProfile;
      return saveCallReply(memory, message, `మీరు ${getSchemeDisplayName(selectedScheme)} గురించి అడిగారు. ${questions[0].question}`);
    }

    const currentQuestion = questions[currentIndex];
    if (currentQuestion) userProfile.callAnswers[currentQuestion.key] = mentionedState || message;

    const nextIndex = currentIndex + 1;
    if (nextIndex < questions.length) {
      userProfile.callQuestionIndex = nextIndex;
      callSessionProfiles[sessionId] = userProfile;
      return saveCallReply(memory, message, `ధన్యవాదాలు. ${questions[nextIndex].question}`);
    }

    userProfile.callQuestionIndex = questions.length;
    callSessionProfiles[sessionId] = userProfile;
    return saveCallReply(memory, message, `మీ వివరాలు నమోదు చేశాను. ${getSchemeDisplayName(selectedScheme)} కోసం తుది అర్హతను అధికారిక పోర్టల్‌లో తనిఖీ చేసి దరఖాస్తు చేయండి: ${selectedScheme.official_link}`);
  }
...
function saveCallReply(memory, message, text) {
  memory.push({ role: 'user', parts: [{ text: message }] });
  memory.push({ role: 'model', parts: [{ text }] });
  return { text, source: 'Call Agent Intake' };
}

function getRequestedCallScheme(message) {
  const value = message.toLowerCase();
  if (value.includes('health') || value.includes('hospital') || value.includes('ayushman') || value.includes('ఆరోగ్య') || value.includes('వైద్య')) {
    return 'ayushman-bharat';
  }
  if (value.includes('kisan') || value.includes('pm kisan') || value.includes('పి యం కిసాన్') || value.includes('పీఎం కిసాన్') || value.includes('రైతు') || value.includes('వ్యవసాయ')) {
    return 'pm-kisan';
  }
  if (value.includes('sukanya') || value.includes('ssy') || value.includes('సుకన్య') || value.includes('అమ్మాయి') || value.includes('కూతురు')) {
    return 'sukanya-samriddhi';
  }
  if (value.includes('mudra') || value.includes('pmmy') || value.includes('ముద్ర') || value.includes('వ్యాపార') || value.includes('రుణ')) {
    return 'pm-mudra';
  }
  if (value.includes('awas') || value.includes('pmay') || value.includes('housing') || value.includes('house') || value.includes('ఆవాస్') || value.includes('ఇల్లు') || value.includes('గృహ')) {
    return 'pm-awas-yojana';
  }
  return null;
}

function getEligibilityQuestions(scheme) {
  const rules = scheme.eligibility || {};
  const questions = [];

  if (rules.farmer_only) {
    questions.push({ key: 'farmer', question: 'మీరు సొంత వ్యవసాయ భూమి ఉన్న రైతునా? అవును లేదా కాదు అని చెప్పగలరా?' });
  }
  if (rules.age_min !== null && rules.age_min !== undefined) {
    questions.push({ key: 'age', question: 'మీ వయస్సు ఎంత ఉందో చెప్పగలరా?' });
  }
  if (rules.gender && rules.gender !== 'Any') {
    questions.push({ key: 'gender', question: 'దరఖాస్తుదారుడి లింగం ఏమిటి?' });
  }
  if (rules.income_max !== null && rules.income_max !== undefined) {
    questions.push({ key: 'income', question: 'మీ కుటుంబ వార్షిక ఆదాయం ఎంత ఉందో చెప్పగలరా?' });
  }
  if (Array.isArray(rules.occupation) && !rules.occupation.includes('Any')) {
    questions.push({ key: 'occupation', question: 'మీ వృత్తి ఏమిటి?' });
  }
  // State is collected consistently for every scheme, even when it is available nationwide.
  questions.push({ key: 'state', question: 'మీరు ఏ రాష్ట్రంలో నివసిస్తున్నారో చెప్పగలరా?' });
  return questions;
}

function getSchemeDisplayName(scheme) {
  const names = {
    'pm-kisan': 'పీఎం కిసాన్ పథకం',
    'ayushman-bharat': 'ఆయుష్మాన్ భారత్ పథకం',
    'sukanya-samriddhi': 'సుకన్య సమృద్ధి యోజన',
    'pm-mudra': 'ప్రధానమంత్రి ముద్ర యోజన',
    'pm-awas-yojana': 'ప్రధానమంత్రి ఆవాస్ యోజన'
  };
  return names[scheme.id] || scheme.name;
}

function getMentionedIndianState(message) {
  const value = message.toLowerCase();
  const states = [
    ['telangana', 'Telangana'],
    ['తెలంగాణ', 'Telangana'],
  ...
  const match = states.find(([name]) => value.includes(name));
