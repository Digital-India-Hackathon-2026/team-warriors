...
const renderTextWithLinks = (text) => String(text).split(/(https?:\/\/[^\s]+)/g).map((part, index) =>
  /^https?:\/\//.test(part) ? (
    <a
      key={`${part}-${index}`}
      href={part}
      ...
      style={{ color: 'var(--accent-color)', fontWeight: 700, textDecoration: 'underline', overflowWrap: 'anywhere' }}
    >
      {part}
    </a>
  ) : part
...
const CallAgent = () => {
  ...
  const isAgentOutputRef = useRef(false);
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      rec.onresult = (event) => {
        ...
        if (finalTranscript.trim() && !isAgentOutputRef.current && !isRequestInFlightRef.current) {
          const text = finalTranscript.trim();
          const previous = lastFinalTranscriptRef.current;
          // Speech recognition can emit the same final phrase twice. Ignore it.
          if (previous.text === text && Date.now() - previous.time < 3000) return;
          lastFinalTranscriptRef.current = { text, time: Date.now() };
          setLiveCaption('');
      ...
      rec.onend = () => {
        ...
        if (callState === 'active' && !isMuted && !isAgentOutputRef.current) {
          setTimeout(() => {
            try {
              rec.start();
              setIsListening(true);
  ...
  const speakTelugu = useCallback((text) => {
    ...
    if (teluguVoice) {
      ...
      setVoiceNotice('Telugu voice is not installed in this browser/device. Install a Telugu text-to-speech voice for spoken Telugu.');
    }

    utterance.onstart = () => setIsAgentSpeaking(true);
    utterance.onend = () => {
      setIsAgentSpeaking(false);
      isAgentOutputRef.current = false;
  ...
  const sendToAgent = useCallback(async (text) => {
    ...
    // Stop recognition before the reply begins so the device never transcribes the agent's own voice.
    isAgentOutputRef.current = true;
    if (recognitionRef.current) {
      try { recognitionRef.current.stop(); } catch (_) {}
      setIsListening(false);
  ...
  return (
    <div style={{ 
      ...
      {voiceNotice && (
        <div role="status" style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', backgroundColor: 'var(--warning-light)', color: 'var(--warning)', fontSize: '12px', textAlign: 'center' }}>
          {voiceNotice}
        </div>
      )}
