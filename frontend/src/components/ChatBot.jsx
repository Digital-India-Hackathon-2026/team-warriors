...
// Set VITE_API_URL in a frontend .env file when the API is deployed elsewhere.
...
const renderTextWithLinks = (text) => String(text).split(/(https?:\/\/[^\s]+)/g).map((part, index) =>
  /^https?:\/\//.test(part) ? (
    <a
      ...
      style={{ color: 'inherit', fontWeight: 700, textDecoration: 'underline', overflowWrap: 'anywhere' }}
    >
      {part}
    </a>
  ) : part
...
const ChatBot = () => {
  ...
  const sendMessage = async (customText = '') => {
    ...
    try {
      ...
      setError(err.message || 'Unable to connect to Scheme Sathi AI. Please try again.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div style={{
            ...
            {messages.length === 1 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {['Find schemes for me', 'Check my eligibility', 'What documents do I need?'].map(prompt => (
                  <button
                    key={prompt}
                    type="button"
                    onClick={() => sendMessage(prompt)}
                    disabled={loading}
                    style={{ padding: '7px 10px', borderRadius: '14px', fontSize: '11px', backgroundColor: 'var(--accent-light)', color: 'var(--accent-color)', border: '1px solid var(--accent-color)' }}
                  >
                    {prompt}
                  </button>
                ))}
            ...
            {error && (
              <div role="alert" style={{ fontSize: '12px', color: 'var(--danger)', padding: '8px 10px', backgroundColor: 'var(--danger-light)', borderRadius: '8px' }}>
                {error}
              </div>
            )}
